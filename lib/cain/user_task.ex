defmodule Cain.UserTask do
  ...
  def claim(%State{assignee: assignee}, _user_id) when not is_nil(assignee) do
    {:already_claimed_by, assignee}
  end

  def claim(%State{id: id, assignee: nil}, user_id) do
  ...
  def unclaim(%State{assignee: nil}) do
    :not_claimed
  end

  def unclaim(%State{id: id}) do
    Task.unclaim(id)
  end
  ...
  def complete(%State{id: task_id}, params \\ %{}, opts \\ []) do
    with_variables_in_return? = Keyword.get(opts, :with_variables_in_return?, true)
  ...
  @spec trigger_bpmn_error(user_task, String.t(), String.t(), Variable.t()) ::
          :ok | {:error, String.t()}
  def trigger_bpmn_error(%State{id: task_id}, error_code, error_message, variables) do
